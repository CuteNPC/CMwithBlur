...
class Consistency(LightningModule):
    def __init__(
        ...
        *,
        ...
        save_samples_every_n_epochs: int = 10,
        num_samples: int = 16,
        sample_steps: int = 1,
        use_ema: bool = True,
        sample_seed: int = 0,
        ...
        sigma_blur_max: Optional[float] = None,
        cond_classes: Optional[int] = None,
        **kwargs,
    ) -> None:
        super().__init__()
        ...
        self.save_samples_every_n_epoch = save_samples_every_n_epochs
        ...
        self.dct_blur = DCTBlur(self.sigma_blur_max, self.image_size)
    ...
    def forward(
        ...
        return self._forward(self.model, images, labels, times)
    ...
    def _forward(
        ...
        out: UNet2DOutput = model(images, times, labels)
    ...
    def training_step(self, images: torch.Tensor, *args, **kwargs):
        ...
        current_noise_image = self.dct_blur(images, timesteps, _bins) + self.image_time_product(noise, current_times,)
        next_noise_image = self.dct_blur(images, timesteps + 1, _bins) + self.image_time_product(noise, next_times,)
        ...
        loss = self.loss_fn(self(next_noise_image, labels, next_times), target)
    ...
    def sample(
        ...
        if self.cond_classes:
            labels = [num % self.cond_classes for num in range(num_samples)]
